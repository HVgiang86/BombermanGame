package BombermanGame.Models.Entities.Characters.AI;

import BombermanGame.Utils.MyLibrary.Pair;
import BombermanGame.Utils.MyLibrary.Queue;
...
public abstract class AI {
    ...
    private void calcDistanceFromEnemy() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (map[u][v] == '2' || map[u][v] == '4' || map[u][v] == '5' || map[u][v] == '6' || map[u][v] == '7' || map[u][v] == '#' || map[u][v] == '*')
                    continue;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                int v = top.getY() + hY[i];
                if (!validate(u, v)) continue;
                if (map[u][v] == '2' || map[u][v] == '4' || map[u][v] == '5' || map[u][v] == '6' || map[u][v] == '7' || map[u][v] == '#')
                    continue;
    ...
    public void calcCurrentMap() throws NullPointerException {
        if (this.gameBoard == null) throw new NullPointerException();
        char[][] temp = this.gameBoard.reviveMap();
        if (temp == null) {
            map = null;
            return;
        }
